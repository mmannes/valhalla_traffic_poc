...
COPY brazil-latest.osm.pbf valhalla_tiles/br.osm.pbf
...
#RUN cd valhalla_tiles; wget --no-check-certificate https://download.geofabrik.de/south-america/brazil-latest.osm.pbf -O br.osm.pbf
...
RUN cd valhalla_tiles; valhalla_build_tiles -c valhalla.json br.osm.pbf
...
#    tile=`valhalla_traffic_demo_utils --get-traffic-dir $edge_id | cut -d '.' -f1`; \
#    valhalla_traffic_demo_utils --config /valhalla_tiles/valhalla.json --generate-live-traffic 0/001/382,20,`date +%s`
